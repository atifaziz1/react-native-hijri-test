...
import com.hijritest.rnHijriDatePicker.time.RadialPickerLayout.OnValueSelectedListener;
...
public class TimePickerDialog extends DialogFragment implements
    ...
    /**
     * Set whether the picker should dismiss itself when it's pausing or whether it
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        int viewRes = mVersion == Version.VERSION_1 ? R.layout.hdp_mdtp_time_picker_dialog
                : R.layout.hdp_mdtp_time_picker_dialog_v2;
    ...
    public boolean isOutOfRange(Timepoint current, int index) {
        ...
        if (index == HOUR_INDEX) {
            if (mMinTime != null && mMinTime.getHour() > current.getHour())
                return true;

            if (mMaxTime != null && mMaxTime.getHour() + 1 <= current.getHour())
                return true;
    ...
    public boolean isAmDisabled() {
        ...
        if (mMinTime != null && mMinTime.compareTo(midday) > 0)
            return true;
    ...
    public boolean isPmDisabled() {
        ...
        if (mMaxTime != null && mMaxTime.compareTo(midday) < 0)
            return true;
    ...
    public Timepoint roundToNearest(@NonNull Timepoint time, @Nullable Timepoint.TYPE type) {
        ...
        if (mMinTime != null && mMinTime.compareTo(time) > 0)
            return mMinTime;

        if (mMaxTime != null && mMaxTime.compareTo(time) < 0)
            return mMaxTime;
        if (mSelectableTimes != null) {
            ...
            for (Timepoint t : mSelectableTimes) {
                ...
                if (type == Timepoint.TYPE.HOUR && t.getHour() != time.getHour())
                    continue;
    ...
    /**
     * Try to start keyboard mode with the specified key, as long as the timepicker
     * is not in the
     * middle of a touch-event.
     *
     * @param keyCode The key to use as the first press. Keyboard mode will not be
     *                started if the
     *                key is not legal to start with. Or, pass in -1 to get into
     *                keyboard mode without a starting
     *                key.
     */
    ...
    private boolean addKeyIfLegal(int keyCode) {
        // If we're in 24hour mode, we'll need to check if the input is full. If in
        // AM/PM mode,
    ...
    /**
     * Traverse the tree to see if the keys that have been typed so far are legal as
     * is,
    ...
    private boolean isTypedTimeFullyLegal() {
        if (mIs24HourMode) {
            // For 24-hour mode, the time is legal if the hours and minutes are each legal.
            // Note:
    ...
    /**
     * Update the hours, minutes, seconds and AM/PM displays with the typed times.
     * If the typedTimes
     * is empty, either show an empty display (filled with the placeholder text), or
     * update from the
     * timepicker's values.
     *
     * @param allowEmptyDisplay if true, then if the typedTimes is empty, use the
     *                          placeholder text.
    ...
    /**
     * Get the currently-entered time, as integer values of the hours, minutes and
     * seconds typed.
     *
     * @param enteredZeros A size-2 boolean array, which the caller should
     *                     initialize, and which
     *                     may then be used for the caller to know whether zeros had
     *                     been explicitly entered as either
     *                     hours of minutes. This is helpful for deciding whether to
     *                     show the dashes, or actual 0's.
     * @return A size-3 int array. The first value will be the hours, the second
     *         value will be the
     *         minutes, and the third will be either TimePickerDialog.AM or
     *         TimePickerDialog.PM.
     */
    ...
    private void generateLegalTimesTree() {
        ...
        if (mIs24HourMode) {
            ...
            // When the first digit is 0-1, and the second digit is 0-5, the third digit may
            // be 6-9.
            ...
            // The time must be finished now, when seconds are disabled. E.g. 10:49am,
            // 12:40pm.
            ...
            // When the first digit is 1, and the second digit is 0-2, and the third digit
            // is 6-9,
            ...
            // When the first digit is 1, and the second digit is 3-5, and the third digit
            // is 0-9,
            ...
            // When the first digit is 2-9, and the second digit is 0-5, the third digit may
            // be 0-9.
            thirdDigit = new Node(k0, k1, k2, k3, k4, k5, k6, k7, k8, k9);
            secondDigit.addChild(thirdDigit);
            ...
            // When the first digit is 2-9, and the second digit is 0-5, and third digit is
            // 0-9, we
